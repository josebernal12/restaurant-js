...
export const createProductInventoryController = async (req, res) => {
  const { name, stock, max, min, unit, companyId,unitQuantity } = req.body
  const product = await createProductInventory(name, stock, max, min, unit, companyId,unitQuantity)
  if (product?.msg) {
...
export const updateProductInventoryController = async (req, res) => {
  const { id } = req.params
  const { name, stock, max, min, unit, companyId,unitQuantity } = req.body
  const product = await updateProductInventory(id, name, stock, max, min, unit, companyId,unitQuantity)
  if (product?.msg) {
    res.status(404).json(product)
    return
  }
